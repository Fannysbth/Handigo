...
const CompleteProfilePage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Profile lengkap! Silakan login.');
      navigate('/login');
    } catch (err) {
      toast.error('Gagal lengkapi profile');
    } finally {
        ...
        <div className="w-full max-w-md bg-light-blue rounded-2xl shadow-lg p-5 sm:p-8">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div>
              ...
              <input value={email} disabled className="w-full mt-1 px-4 py-2 rounded-full bg-gray-100" />
            </div>
            <div>
              <label className="text-xs text-primary-blue">Nama Lengkap</label>
              <input value={full_name} disabled className="w-full mt-1 px-4 py-2 rounded-full bg-gray-100" />
            </div>
            <div>
